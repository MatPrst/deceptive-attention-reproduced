...
def initialize_sentences(task, debug, num_train, splits, data_path):
    sentences = []

    for sp in splits:
        src_filename = data_path + sp + "." + task + ".src"
        trg_filename = data_path + sp + "." + task + ".trg"

        src_sentences = open(src_filename).readlines()
        trg_sentences = open(trg_filename).readlines()

        alignment_filename = data_path + sp + "." + task + ".align"
